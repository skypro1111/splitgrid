#!/bin/sh
# splitgrid agent terminal control (POSIX). Invoked by an agent as:
#   splitgrid-terminal.sh <cmd> [args...]
#   read:   list | read <id> [--tail N] | info <id> | tree <id>
#   act:    send <id> <text...> | type <id> <text...> | key <id> <name>
#   run `splitgrid-terminal.sh help` for the full list.
# Lets an agent inspect and drive the OTHER terminals in its workspace. Forwards
# the argv to splitgrid (tagged with $SPLITGRID_TERMINAL + $SPLITGRID_TERMINAL_TOKEN) and
# PRINTS the JSON reply so the agent can read the result. No-op outside splitgrid.
[ -n "$SPLITGRID_TERMINAL" ] || { echo '{"ok":false,"error":"not_in_splitgrid"}'; exit 0; }
endpoint="${SPLITGRID_TERMINAL_ENDPOINT:-http://127.0.0.1:19558/terminal}"
...
if [ -z "$use_bridge" ] && [ -n "$WSL_DISTRO_NAME" ]; then
  ...
  if [ "$host" = "127.0.0.1" ] || [ "$host" = "localhost" ]; then
    ...
    # /tmp: this value is the host we POST $SPLITGRID_TERMINAL_TOKEN to, so a planted
...
payload="{\"kind\":\"terminal\",\"terminal\":\"$SPLITGRID_TERMINAL\",\"token\":\"$SPLITGRID_TERMINAL_TOKEN\",\"argv\":[$argv]}"

#!/bin/sh
# splitgrid agent browser control (POSIX). Invoked by an agent as:
#   splitgrid-browser.sh <cmd> [args...]
#   read:   open <url> | navigate <url> | get url|title|text|html|console | screenshot | eval <js>
#   act:    snapshot | click <ref> | fill <ref> <text> | type <text> | select <ref> <value>
#   sync:   wait load | wait selector <css> | wait text <substr> | wait <ms>
#   net:    network start | network list | network clear | network stop
#   run `splitgrid-browser.sh help` for the full list.
# Forwards the argv to splitgrid (tagged with $SPLITGRID_TERMINAL + $SPLITGRID_BROWSER_TOKEN)
...
endpoint="${SPLITGRID_BROWSER_ENDPOINT:-http://127.0.0.1:19558/browser}"
...
# (refused probes return instantly, so this stays fast). Shared with splitgrid-hook.sh.
if [ -z "$use_bridge" ] && [ -n "$WSL_DISTRO_NAME" ]; then
  ...
  if [ "$host" = "127.0.0.1" ] || [ "$host" = "localhost" ]; then
    ...
    # /tmp: this value is the host we POST $SPLITGRID_BROWSER_TOKEN to, so a planted
...
payload="{\"kind\":\"browser\",\"terminal\":\"$SPLITGRID_TERMINAL\",\"token\":\"$SPLITGRID_BROWSER_TOKEN\",\"argv\":[$argv]}"

if [ -n "$use_bridge" ]; then
  # Write the request atomically (.tmp → rename), then poll for the reply file.
  # Bridge processes within its own 35s budget; wait a touch longer (40s) so we
